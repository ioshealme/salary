...
int main()
{
	...
	do
	{
		...
		if(n<7||n>7)
			...
			case 5:write();
			case 6:list();
			case 7:printf("退出系统，欢迎下次光临!\n");
			default:exit(0);//退出系统，exit(0)函数有退出功能。
			}
...
void list()// 
{
	...
	printf("浏览员工的数据");
	for(i=0;fread(&zggz[i],sizeof(struct member),1,fp)!==NULL;i++)
	{
		    printf("%6d",i+1);
...
void modify()                  //指定工号，修改该职工的工资记录，其中要调用grsds()函数计算个人所得税。
{
	...
	for(i=0;i<=n;i++)
	{
		 ...
		 printf("%s",zggz[i].name);
		  ...
		  zggz[i].yfsalsry=zggz[i].gwsalary+zggz[i].xisalary+zggz[i].zwsalary+zggz[i].jxsalary;
		  zggz[i].grsds=grsds(zggz[i].yfsalsry);
		  zggz[i].sfsalary=zggz[i].yasalary+zggz[i].grsds;
